...
// 임시 통신 주소 로그인 키
const accessToken =
    'eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ5ZWppIiwiYXV0aCI6IlVTRVIiLCJuYW1lIjoi7JiI7KeAIiwicGhvbmUiOiIwMTAtMDAwMC0wMDAwIiwiZXhwIjoxNjk1ODgyMDcxfQ.XgYC2up60frNzdg8TMJ3nC3JRRwFFZiBFXTE0XRTmS4';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomBtn(
        text: '다음',
        action: () {
          goNext(context, TermsAndConditionsPage());
        },
        isDisabled: false,
      ),
    );
                      ...
                      Expanded(
                          child: Form(
                              ...
                              child: renderTextFormField(
                                  label: '전화번호',
                                  onSaved: (val) async {
                                    var formattedPhone = val.replaceAllMapped(
                                        RegExp(r'(\d{3})(\d{3,4})(\d{4})'),
                                        (m) => '${m[1]}-${m[2]}-${m[3]}');
                                    final response = await dioPost(
                                        accessToken: accessToken,
                                        url:
                                            '/bank-service/account/phone-check/yoonyeji',
                              ...
                              child: renderTextFormField(
                                  ...
                                  onSaved: (val) async {
                                    final response = await dioPost(
                                        accessToken: accessToken,
                                        url:
                                            '/bank-service/account/phone-auth/yoonyeji',
                                        data: {'code': val});
                                    if (response != null) {
                                      Navigator.push(
                                          context,
      ...
      bottomNavigationBar: BottomBtn(
        ...
        action: () async {
          if (widget._verificationFormKey.currentState != null &&
              widget._verificationFormKey.currentState!.validate()) {
        ...
        isDisabled: _phoneAuthenticationResult == true &&
